...
class MonitoringService(AbstractMonitoringService):
    ...
    def _setup_metrics(self):
        ...
        self.requests_counter = self.meter.create_counter(
            "llm_requests_total", description="Total LLM requests", unit="requests"
        )

        self.tokens_counter = self.meter.create_counter(
            "llm_tokens_total", description="Total tokens processed", unit="tokens"
        )

        self.cost_counter = self.meter.create_counter(
            "llm_cost_total", description="Total LLM cost in USD", unit="USD"
        )

        self.latency_histogram = self.meter.create_histogram(
            "llm_latency_ms", description="LLM request latency", unit="ms"
        )

        self.cache_hits_counter = self.meter.create_counter(
            "cache_hits_total", description="Total cache hits", unit="hits"
        )

        self.cache_misses_counter = self.meter.create_counter(
            "cache_misses_total", description="Total cache misses", unit="misses"
        )
    ...
    async def record_cache_hit(self, cache_key: str) -> None:
        ...
        try:
            self.cache_hits_counter.add(
                1, {"cache_key_prefix": cache_key.split(":")[0]}
            )
        except Exception as e:
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_tokens_per_request": self._total_tokens / self._total_requests
            if self._total_requests > 0
            else 0,
        }
